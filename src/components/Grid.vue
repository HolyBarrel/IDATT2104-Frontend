...
async function sendNetwork() {
  ...
  socket.send(
    '[{"x":0,"y":0,"landscape":"' +
      tiles.value[0][0].landscape +
      '","building":"' +
      tiles.value[0][0].building +
      '"}]'
  )
}
...
function paintBucket(x, y, originalLandscape) {
  ...
  if (
    x < 0 ||
    y < 0 ||
    x >= tiles.value.length ||
    y >= tiles.value[0].length ||
    tiles.value[x][y].landscape != originalLandscape
  ) {
    return
  }
...
<template>
  ...
  <div class="grid" @mousedown="mouseDown = true" @mouseup="mouseUp" @mouseleave="mouseUp">
    <div v-for="row in tiles" class="row">
      <Tile
        v-for="tile in row"
        :tile="tile"
        @mouseover="tileClick(tile.x, tile.y, false)"
        @click="tileClick(tile.x, tile.y, true)"
        @contextmenu.prevent="selectedTile = tile"
      />
    </div>
  </div>
  ...
  <div class="welcome-box" v-if="displayWelcome" @click="displayWelcome = false">
    <div class="welcome-box-content">
      ...
      <p>
        Here you can create your own maps for the game. You can place cell towers, signal extenders,
        and paint the landscape.
      </p>
      <p>
        After each edits, the map is sent to the server, where it calculates the signal strength for
        each tile. This can take a few seconds.
      </p>
      <p>You can right-click on a tile to see detailed information about it.</p>
      <p>Have fun!</p>
    </div>
  </div>
